...
namespace questvault.Controllers
{
    ...
    ) : Controller
  {
    ...
    public async Task<IActionResult> Index(string releaseStatus, string genre, string releasePlatform)
    {
      ...
      {
        var filteredGames = await context.Games.OrderByDescending(
    ...
    public async Task<IActionResult> ResultsPost(string searchTerm)
    {
      ...
      foreach (var game in games)
      {
        ...
        var existingGame = context.Games.FirstOrDefault(g => g.IgdbId == game.IgdbId);
    ...
    private async Task ProcessGameCompaniesAsync(Models.Game game, Models.Game newGame)
    {
      ...
      var companies = await igdbService.GetCompaniesFromIds(companyIds);

      foreach (var company in companies)
      {
        var existingCompany = context.Companies.FirstOrDefault(c => c.IgdbCompanyId == company.IgdbCompanyId);
        ...
        context.GameCompany.Add(gameCompany);
      }
    }
    ...
    private async Task ProcessGamePlatformsAsync(Models.Game game, Models.Game newGame)
    {
      ...
      var platforms = await igdbService.GetPlatformsFromIds(platformIds);

      foreach (var platform in platforms)
      {
        var existingPlatform = context.Platforms.FirstOrDefault(p => p.IgdbPlatformId == platform.IgdbPlatformId);
        ...
        context.GamePlatform.Add(gamePlatform);
